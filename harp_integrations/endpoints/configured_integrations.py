...
from harp_integrations.models.configured_integrations import Integrations, IntegrationsSchema
...
logger = get_logger()
ns = api.namespace('api/v1/integrations/configured', description='Harp All Configured integrations endpoints')
    ...
    def put():
        ...
        * Send a JSON object
        ...
        try:
            ...
            data['integration_key'] = uuid.uuid1()
...
@ns.route('/integration_key/<integration_key>')
class GetIntegrationByKey(Resource):
    @staticmethod
    def get(integration_key):
        """
            Return Integrations object with specified key
        """
        if not integration_key:
            return {'msg': 'integration_key should be specified'}, 404
        obj = Integrations.obj_exist(integration_key=integration_key)
        if not obj:
            return {'msg': f'object with integration_key - {integration_key} is not found'}, 404
        result = integrations.dump(obj.dict())
        return result, 200


@ns.route('/all')
class AllConfiguredIntegrations(Resource):
    @staticmethod
